...
describe('MySQL ALTER TABLE key and charset actions', () => {
  it('keeps ADD KEY actions without inserting COLUMN', () => {
    const out = formatSQL('ALTER TABLE t ADD KEY idx (col);', { dialect: 'mysql' });
  ...
  it('keeps ADD INDEX actions without inserting COLUMN', () => {
    const out = formatSQL('ALTER TABLE t ADD INDEX idx (col);', { dialect: 'mysql' });
  ...
  it('keeps ADD FULLTEXT KEY actions without inserting COLUMN', () => {
    const out = formatSQL('ALTER TABLE t ADD FULLTEXT KEY idx (col);', { dialect: 'mysql' });
  ...
  it('parses key and charset ALTER statements in strict mode', () => {
    expect(() => parse('ALTER TABLE t ADD KEY idx (col);', { dialect: 'mysql', recover: false })).not.toThrow();
    expect(() => parse('ALTER TABLE t1 MODIFY col1 VARCHAR(100) CHARACTER SET utf8mb4;', { dialect: 'mysql', recover: false })).not.toThrow();
  });
});
